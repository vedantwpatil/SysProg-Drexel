...
int exec_remote_cmd_loop(char *address, int port) {
  ...
  while (1) {
    ...
    if (strcmp(cmd_buff, "exit") == 0 || strcmp(cmd_buff, "stop-server") == 0) {
      ...
      if (io_size > 0) {
        printf("%.*s", (int)io_size - 1, rsp_buff); // Skip EOF char
      }
      break;
    }
    ...
    while (1) {
      ...
      // Print response (without EOF if present)
      if (is_eof) {
 ...
 *              RDSH_DEF_PORT which is 1234 in rshlib.h.  If you are using
 *              tux you may need to change this to your own default, or even
 *              better use the command line override -c implemented in
 * dsh_cli.c For example ./dsh -c 10.50.241.18:5678 where 5678 is the new port
