...
int get_student(int fd, int id, student_t *s) {
  ...
  student_t empty = {0};
  return (memcmp(s, &empty, STUDENT_RECORD_SIZE) != 0) ? NO_ERROR
                                                       : SRCH_NOT_FOUND;
}
...
int del_student(int fd, int id) {
  student_t target;
  if (get_student(fd, id, &target) != NO_ERROR) {
  ...
  if (write(fd, &empty, STUDENT_RECORD_SIZE) != STUDENT_RECORD_SIZE) {
    printf(M_ERR_DB_WRITE);
    return ERR_DB_FILE;
  }
...
int count_db_records(int fd) {
  ...
  while (read(fd, &record, STUDENT_RECORD_SIZE) == STUDENT_RECORD_SIZE) {
    student_t empty = {0};
    if (memcmp(&record, &empty, STUDENT_RECORD_SIZE) != 0)
      count++;
  }
...
int print_db(int fd) {
  ...
  while (read(fd, &record, STUDENT_RECORD_SIZE) == STUDENT_RECORD_SIZE) {
    student_t empty = {0};
    if (memcmp(&record, &empty, STUDENT_RECORD_SIZE)) {
      if (!header_printed) {
        printf(STUDENT_PRINT_HDR_STRING, "ID", "FIRST_NAME", "LAST_NAME",
               "GPA");
        header_printed = true;
      }
      has_records = true;
      printf(STUDENT_PRINT_FMT_STRING, record.id, record.fname, record.lname,
             record.gpa / 100.0f);
    }
  }
...
int compress_db(int fd) {
  ...
  // Create temporary database file with truncation
  ...
  // Read through original database
  lseek(fd, 0, SEEK_SET);
  while (read(fd, &record, STUDENT_RECORD_SIZE) == STUDENT_RECORD_SIZE) {
    // Only write non-empty records
    if (memcmp(&record, &empty, STUDENT_RECORD_SIZE) != 0) {
      if (write(tmp_fd, &record, STUDENT_RECORD_SIZE) != STUDENT_RECORD_SIZE) {
        printf(M_ERR_DB_WRITE);
        close(tmp_fd);
        return ERR_DB_FILE;
      }
