...
// The compiler requires there to be a prototype for the function with the more
// in depth definitions to be listed further below
void usage(char *);
...
void reverse_string(char *str) {
  ...
  while (end_idx > start_idx) {
    char first = str[start_idx];
    char second = str[end_idx];

    str[start_idx] = second;
    str[end_idx] = first;
    start_idx++;
    end_idx--;
  }
...
//  in a word.
//  Now, at this point where we are incrementing wc we need to do a few more
//  things
//       1. incrment wc, and set word_start to true like before
//       2. Now, set wlen to zero, as we will be counting characters in each
//       word
//       3. Since we are starting a new word we can printf("%d. ", wc);
//
//  If word_start is true, we are in an active word, so each time through the
//  loop we would want to:
//       1. Check if the current character is not a SPACE_CHARACTER
//          a.  IF it is NOT A SPACE -> print the current character, increment
//          wlen
//
//       2.  In the loop there are 2 conditions that indicate a current word is
//       ending:
//           a. word_start is false and the current character is a
//           SPACE_CHARACTER
//                   OR
//           b. the current loop index is the last character in the string (aka
//           the
...
void word_print(char *str) {
  ...
  for (int i = 0; i < last_char_idx; i++) {
    ...
    if (current != SPACE_CHAR && !word_start) {
      wc++;
      word_start = true;
      wlen = 0;
      printf(" (%d)\n", wlen);
    ...
    if ((current == SPACE_CHAR) || (1 == last_char_idx)) {
      printf(" (%d\n", wlen);
      word_start = false;
      wlen = 0;
    }
...
int main(int argc, char *argv[]) {
  ...
  //           A default option is required for edge cases, ensuring that all
  //           the possible cases are accounted for
  default:
    usage(argv[0]);
